...
PiPER-Mate 是一个机械臂遥操作控制项目，支持通过 **FashionStar 机械臂** 实时远程控制 **Piper 机械臂**。项目提供三种控制方式，适用于机器人研究、遥操作教学、AI训练数据采集等多种场景。

### ✨ 核心特性

- 🤖 **多控制方式**：支持 ROS2 HUMBLE、Lerobot 框架、Python SDK 三种控制模式
- ⚡ **高控制频率**：支持最高 100Hz 的实时遥操作
- 🔄 **关节自动映射**：自动将 FashionStar 关节角度转换为 Piper 控制指令
...
| 操作系统 | Ubuntu 22.04 |
| ROS版本 | ROS2 Humble |
| 硬件设备 | FashionStar 机械臂 + Piper 机械臂 |
...
│   ├── src/piper/               # Piper驱动节点
│   ├── src/piper_mate/          # FashionStar驱动节点
...
系统自动将 FashionStar 的 6 个关节映射到 Piper 机械臂：

| 关节 | FashionStar 角度 | Piper 弧度 | 方向 |
|------|------------------|------------|------|
...
程序已添加异常处理，当FashionStar USB断开时会自动终止并显示错误信息：

```bash
❌ 致命错误：FashionStar USB连接断开！
```
...
- **感谢**：FashionStar 和 Piper 团队的硬件支持

---

## 🔗 相关链接

- [FashionStar 官方仓库](https://github.com/servodevelop)
